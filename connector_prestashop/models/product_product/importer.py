    ...
    def set_variant_images(self, combinations):
        ...
        for combination in combinations:
            try:
                ...
                if 'id' in images[0]:
                    img_ids = [
                        binder.to_odoo(x.get('id'), unwrap=True).id for x in
                        images]
                else:
                    img_ids = []
                if img_ids:
...
class ProductCombinationMapper(ImportMapper):
    ...
    def product_tmpl_id(self, record):
        template = self.get_main_template(record)
        return {'product_tmpl_id': template.odoo_id.id}

    @mapping
    def from_main_template(self, record):
        main_template = self.get_main_template(record)
        result = {}
    ...
    def main_template(self, record):
        ...
        return self.env['prestashop.product.template'].browse(template_id)
    ...
    def _get_option_value(self, record):
        ...
        for option_value in option_values:
            option_value_binder = self.binder_for(
                'prestashop.product.combination.option.value')
            option_value_odoo = option_value_binder.to_odoo(
                option_value['id'])
            yield option_value_odoo
    ...
    def main_template_id(self, record):
        return {'main_template_id': self.get_main_template(record).id}
...
@prestashop
class ProductCombinationOptionValueRecordImport(TranslatableRecordImporter):
    _model_name = 'prestashop.product.combination.option.value'
